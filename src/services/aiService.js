...
const { fetchMacroContext, formatMacroBlock } = require('./marketContext');
...
// [A] 고정 역할 정의 — 데이터 분석과 추정/판단을 명확히 분리하는 5섹션 구조 강제
...
[답변 형식 — 반드시 아래 5섹션 구조 준수, 섹션 헤더 그대로 표기]

【1. 현황 요약】
- 현재가, 박스권 위치, 평가손익을 1~2줄로 간결히.

【2. 데이터 기반 분석】 (객관적 사실만, 숫자 근거 명시)
- 지표(RSI/BB%B/OBV/MACD/다이버전스/캔들 패턴) 해석
- 박스권 대비 위치, 거래량 추세
- 검증 가능한 사실만. 추정·심리·전망 금지.

【3. 시장 흐름 / 분위기 / 매크로 추정】 (별도 섹션 — 추정과 판단)
※ 이 섹션은 데이터 분석과 명확히 구분된 추정 영역입니다.
- 한국증시 전반 흐름(KOSPI/KOSDAQ 지수, 외국인/기관 수급 추정)
- 매크로 컨텍스트(미국 금리, VIX, USD/KRW, Fed 정책 분위기)
- 섹터/업종 분위기(예: 보안/반도체/2차전지)
- 투자 심리 + 뉴스 모멘텀 + 종목 특이 이슈
- "~로 보임/~로 추정/~할 가능성" 등 추정 표현 사용. 단언 금지.

【4. 대응 시나리오】 (2~3개)
- 각 시나리오에 진입가/수량/평단 재계산/잔여 자금 명시.
...
【3. 시장 흐름 / 분위기 / 매크로 추정】 (별도 섹션 — 추정과 판단)
※ 이 섹션은 데이터 분석과 명확히 구분된 추정 영역입니다.
- 미국 증시 전반 흐름(S&P/Nasdaq/Dow 추정)
- 매크로 컨텍스트(Fed FOMC/금리/CPI/PCE/NFP/VIX/DXY/10Y 등 거시 변수 분위기)
- 섹터 흐름(반도체/AI/빅테크/금융 등)
- 어닝 시즌 영향 + 종목 특이 이슈
- "~로 보임/~로 추정" 표현 사용. 단언 금지.

【4. 대응 시나리오】 (2~3개)
- 각 시나리오에 진입가/수량/평단 재계산 USD 단위 명시.

【5. 핵심 모니터링 포인트】
- 가격/지표/이벤트 트리거 구체 수치로.
 ...
 */
function buildSystemPrompt({ mode, holdings, stockInfo, latestIndicator, webResults, memoryItems, livePrice, perplexicaResult, macroContext }) {
...
예시: "안랩 박스권 하단을 메모리에 저장해줘" → %%MEM:ADD%%안랩(053800) 박스권 하단 73,000원%%END%%`;
  ...
  // [H] 매크로 / 시장 컨텍스트 — KOSPI/KOSDAQ/USD-KRW/VIX/10Y/DXY (있을 때만)
  // 데이터 분석과 분리된 "시장 흐름/분위기" 추정 섹션의 사실 근거.
  const blockH = macroContext ? formatMacroBlock(macroContext) : '';

  const blocks = [promptA, promptB, blockC, blockD, blockE];
  if (blockF) blocks.push(blockF);
  if (blockH) blocks.push(blockH);
  blocks.push(blockG);
  return blocks.join('\n\n');
}
...
async function chatWithOllama(messages, onChunk, onDone, modelOverride, images) {
  ...
  const body = JSON.stringify({
    ...
    // num_ctx: Ollama 컨텍스트 윈도우 명시 — gemma4:12b 기본 8K 초과 시 0 토큰 응답 사고 방지.
    //         12288 = 안전 한도 (RTX 5060 Ti 16GB 기준 안정 동작).
    options: { temperature: 0.7, top_p: 0.9, num_predict: 4096, num_ctx: 12288 }
  });
...
async function chatWithClaude(systemPrompt, messages, onChunk, onDone, images) {
  ...
  try {
    ...
    const stream = client.messages.stream({
      model,
      // max_tokens 8192 — 5섹션 + 시나리오 + 후속 질문 충분 수용. Sonnet 4.6 한도 내.
      max_tokens: 8192,
      system:     systemPrompt,
    ...
    // max_tokens 도달로 잘렸으면 사용자에게 안내 표시
    if (stopReason === 'max_tokens') {
      const truncNotice = `\n\n⚠ 응답이 8192 토큰 한도에 도달해 잘렸습니다. "이어서 답변해줘"로 계속 받을 수 있습니다.`;
      fullContent += truncNotice;
      onChunk({ content: truncNotice });
    }
    console.log(`[claude] stop_reason=${stopReason} input=${inputTokens} output=${outputTokens}`);

    onDone({ engine: 'claude', tokens: inputTokens + outputTokens, stopReason });
    return fullContent;
  } catch (err) {
...
async function chat({ message, ticker, engine, ohlcvData, model, images, onChunk, onDone, sessionId }) {
  // history limit (2026-06-29 재조정):
  //   Claude: 10 (비용 절감)
  //   Ollama: 15 (gemma4:12b 12K context 한계 — 40턴이면 토큰 폭주로 0 토큰 응답 사고)
  const historyLimit = engine === 'claude' ? 10 : 15;
  ...
  if (shouldSearch) {
    ...
    const baseQuery = triggerHit ? (message.replace(/인터넷(에서)?\s*/g, '').trim() || message) : message;
    // 매크로/시장 흐름 키워드 자동 결합 — 종목 분석에 거시 컨텍스트 결합 유도.
    // 종목명 + 시장흐름 + 매크로 동시 검색 → Perplexica가 거시 변수 포함한 답변 생성.
    const stockName = stockInfo?.name || ticker;
    const macroSuffix = isUsStock
      ? ' (recent US market trend, Fed rate, VIX, sector flow)'
      : ' (최근 한국증시 KOSPI/KOSDAQ 흐름, 외국인 수급, 미국 금리/VIX, 섹터 분위기)';
    const query = `${baseQuery} ${stockName}${macroSuffix}`;

    const pxTimeout = parseInt(process.env.PERPLEXICA_TIMEOUT_MS || '30000', 10);
    onChunk({ content: `🔍 Perplexica 검색: "${query.slice(0, 100)}" (최대 ${Math.round(pxTimeout/1000)}초)...\n\n` });
  ...
  // 매크로 컨텍스트 fetch (10분 캐시) — 실패해도 무시, [H] 블록 생략.
  let macroContext = null;
  try { macroContext = await fetchMacroContext(); }
  catch (e) { console.warn('[chat] macroContext fetch 실패:', e.message); }

  const systemPrompt = buildSystemPrompt({ mode, holdings, stockInfo, latestIndicator, webResults, memoryItems, livePrice, perplexicaResult, macroContext });
